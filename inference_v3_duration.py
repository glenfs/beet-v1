...
HOLD_TOKEN_ID = mappings.get("_")

# Inner-voice decoding controls
BASS_TEMPERATURE = 0.15
ALTO_TEMPERATURE = 0.15
SUSTAIN_BIAS_FACTOR = 2.2
SUSTAIN_STEPS_MIN = 1
SUSTAIN_STEPS_MAX = 3


def get_mode_id(score_or_part):
    try:
        analyzed_key = score_or_part.analyze("key")
        return MODE_BINS - 1 if getattr(analyzed_key, "mode", None) == "major" else 0
    except Exception:
        return 0


def get_original_key_and_time_signature(song):
    """
    Capture original key signature (sharps) and time signature text from input.
    Falls back to analysis/defaults when explicit score symbols are missing.
    """
    key_sig_elements = song.flatten().getElementsByClass(m21.key.KeySignature)
    if key_sig_elements:
        key_signature_sharps = key_sig_elements[0].sharps
    else:
        try:
            key_signature_sharps = song.analyze("key").sharps
        except Exception:
            key_signature_sharps = 0

    ts_elements = song.flatten().getElementsByClass(m21.meter.TimeSignature)
    time_signature_ratio = ts_elements[0].ratioString if ts_elements else "4/4"

    return key_signature_sharps, time_signature_ratio


def get_to_model_interval(song):
    """
    Interval that maps input key to model key space (C major / A minor).
    """
    try:
        analyzed_key = song.analyze("key")
        if getattr(analyzed_key, "mode", None) == "minor":
            target = m21.pitch.Pitch("A")
        else:
            target = m21.pitch.Pitch("C")
        return m21.interval.Interval(analyzed_key.tonic, target)
    except Exception:
        return m21.interval.Interval(m21.pitch.Pitch("C"), m21.pitch.Pitch("C"))


def _is_note_on_token(token):
    return token not in {"_", "r", "/", "^"}
...
def generate_parts(midi_file):
    ...
    key_signature_sharps, time_signature_ratio = get_original_key_and_time_signature(song)

    ##Interval that maps input key to model key space (C major / A minor).
    to_model_interval = get_to_model_interval(song)
    back_to_original_interval = to_model_interval.reverse()

    soprano_part = soprano_part.transpose(to_model_interval)
    ...
    for items in iterator:
        ...
        bass_ids = generate_ids_with_sustain_bias(
            bass_pred,
            token_lookup=value_to_key,
            hold_token_id=HOLD_TOKEN_ID,
            temperature=BASS_TEMPERATURE,
            sustain_bias=SUSTAIN_BIAS_FACTOR,
            sustain_min=SUSTAIN_STEPS_MIN,
            sustain_max=SUSTAIN_STEPS_MAX,
        )
        ...
        alto_ids = generate_ids_with_sustain_bias(
            alto_pred,
            token_lookup=value_to_key,
            hold_token_id=HOLD_TOKEN_ID,
            temperature=ALTO_TEMPERATURE,
            sustain_bias=SUSTAIN_BIAS_FACTOR,
            sustain_min=SUSTAIN_STEPS_MIN,
            sustain_max=SUSTAIN_STEPS_MAX,
        )
    ...
    return save_melody_4parts(
        all_midi_data,
        transpose_interval=back_to_original_interval,
        key_signature_sharps=key_signature_sharps,
        time_signature_ratio=time_signature_ratio,
    )
...
def sample_with_temperature(probabilities, temperature):
    ...
    predictions = np.log(probs) / temperature
...
def generate_ids_with_sustain_bias(
    model_prediction,
    token_lookup,
    hold_token_id,
    temperature=0.15,
    ...
    """
    Sampling with short sustain encouragement for inner voices.
    After a note-on token, bias '_' for the next 1-3 steps.
    """
    sampled_ids = []
    sustain_remaining = 0

    for note_probs in model_prediction[0]:
        probs = np.asarray(note_probs, dtype=np.float64).copy()

        if hold_token_id is not None and sustain_remaining > 0:
            probs[hold_token_id] *= sustain_bias
        ...
        sampled_token = token_lookup.get(int(sampled_id), "")

        if hold_token_id is not None and sampled_id == hold_token_id:
            if sustain_remaining > 0:
                sustain_remaining -= 1
        elif _is_note_on_token(sampled_token):
            sustain_remaining = np.random.randint(sustain_min, sustain_max + 1)
        else:
            sustain_remaining = 0
